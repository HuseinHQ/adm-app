        ...
        <form class="flex items-end ml-6">
          ...
          <div class="pr-3">
            ...
            <select
              ...
              @change="cekAdn"
              autocomplete="off"
              class="border border-slate-500 text-black text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-[200px] px-4 py-[10px]"
            >
              <option disabled value="" class="disabled:text-slate-300">
                Select ADN
              </option>
          ...
          <div class="pr-3">
            ...
            >
              ...
              <option
                v-for="keywordData in itemsKeyword"
                v-bind:value="keywordData.keyword"
              >
                {{ keywordData.keyword }}
              </option>
            </select>
          ...
          <div class="pr-3">
            ...
            <input
              ...
              v-model="startDate"
              type="date"
              class="border border-slate-500 text-slate-300 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-[200px] px-4 py-[8px]"
            />
          </div>
          ...
          <button
            type="submit"
            class="bg-blue-500 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-9 rounded-md h-10"
            v-on:click.prevent="generateTable"
          >
            Generate
          </button>
        </form>
      ...
      <div class="bg-white mx-auto rounded-md shadow-md">
        ...
        <div class="pt-5 mx-6 mb-5 flex gap-3">
          <button
            ...
            v-on:click.prevent="downloadTable"
          >
            Download
          </button>
          <!-- Search -->
...
export default {
  ...
  methods: {
    ...
    async cekAdn() {
      ...
      let cookie = getCookie.split('Session=')
      const response = await axios
        .post('http://localhost:5000/api/v1/getkeyword', {
          cookies: cookie[1],
          ADN_Number: this.selectedData,
        })
        .catch((err) => {
    ...
    filteredItems() {
      return this.paginate(
        this.items.filter((item) => {
          if (
            item.sms.toLowerCase().indexOf(this.searchKey.toLowerCase()) > -1
          ) {
            return (
              item.sms.toLowerCase().indexOf(this.searchKey.toLowerCase()) > -1
            )
            ...
            return (
              item.msisdn.toLowerCase().indexOf(this.searchKey.toLowerCase()) >
              -1
            )
          }
        })
